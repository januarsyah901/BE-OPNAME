...
| BE Dev | Tabel `users`, seed data (admin, kasir, mekanik) | S | `Todo` |
| FE Dev | Halaman Login (UI + auth logic + protected routes) | M | `Done` |
| FE Dev | Sidebar dinamis berdasarkan role | M | `Done` |
| UI/UX | High-fi: Login page + Layout utama | M | `Todo` |

**Deliverable:**
- [ ] Login end-to-end (BE + FE)
- [x] FE: Login UI + sidebar dinamis sudah siap (mock auth)
- [ ] Halaman diproteksi berdasarkan role (BE auth)
...
### 🟦 Sprint 5 — Minggu 5: Customer & Kendaraan
**Goal:** Staff bisa data customer, kendaraan, & lihat histori servis
...
| UI/UX | High-fi: Customer & Kendaraan | M | `Todo` |

**Deliverable:**
- [x] FE: Customer & kendaraan selesai (mock data)
- [ ] API customer & kendaraan (BE)
