...
async function seedUsers() {
    const users = [
        { name: 'Owner Bengkel', username: 'owner', password: 'owner123', role: 'owner' },
        { name: 'Admin Utama', username: 'admin', password: 'admin123', role: 'admin' },
        { name: 'Kasir Satu', username: 'kasir1', password: 'kasir123', role: 'kasir' },
    ];
    ...
    for (const u of users) {
        try {
            ...
            await prisma.users.upsert({
                ...
                update: {},
                create: { name: u.name, username: u.username, password_hash, role: u.role }
            });
        } catch (e: any) {
...
async function seedServiceCatalog() {
    const services = [
        { name: 'Ganti Oli Mesin', description: 'Penggantian oli mesin standar', kategori: 'Mesin', standard_price: 100000, durasi_estimasi: '30 menit', berlaku_untuk: 'keduanya', garansi: null, is_active: true },
        { name: 'Tune Up Lengkap', description: 'Tune up busi, filter udara, karburator', kategori: 'Mesin', standard_price: 250000, durasi_estimasi: '2-3 jam', berlaku_untuk: 'mobil', garansi: '1 bulan', is_active: true },
        { name: 'Ganti Kampas Rem', description: 'Penggantian kampas rem depan/belakang', kategori: 'Rem & Transmisi', standard_price: 150000, durasi_estimasi: '1-2 jam', berlaku_untuk: 'keduanya', garansi: '3 bulan', is_active: true },
        { name: 'Servis AC', description: 'Cuci evaporator, isi freon, cek kompressor', kategori: 'AC & Kabin', standard_price: 350000, durasi_estimasi: '2-3 jam', berlaku_untuk: 'mobil', garansi: null, is_active: true },
        { name: 'Balancing & Spooring', description: 'Penyeimbangan dan penyelarasan roda', kategori: 'Lainnya', standard_price: 200000, durasi_estimasi: '1 jam', berlaku_untuk: 'mobil', garansi: null, is_active: true },
    ];

    for (const s of services) {
        try {
            const existing = await prisma.service_catalog.findFirst({ where: { name: s.name } });
            if (!existing) {
...
async function seedTransactions() {
    const customers = await prisma.customers.findMany({ where: { deleted_at: null }, orderBy: { id: 'asc' } });
    const vehicles = await prisma.vehicles.findMany({ orderBy: { id: 'asc' } });
    const spareParts = await prisma.spare_parts.findMany({ where: { deleted_at: null }, orderBy: { id: 'asc' } });
    const users = await prisma.users.findMany({ where: { deleted_at: null }, orderBy: { id: 'asc' } });

    if (!customers.length || !vehicles.length || !spareParts.length || !users.length) {
    ...
    const txData = [
        {
            ...
            items: [
                { item_type: 'spare_part', spare_part_id: spareParts[0].id, item_name: spareParts[0].name, quantity: 2, unit_price: Number(spareParts[0].sell_price) },
        ...
        },
        {
            ...
            items: [
                { item_type: 'spare_part', spare_part_id: spareParts[0].id, item_name: spareParts[0].name, quantity: 4, unit_price: Number(spareParts[0].sell_price) },
                { item_type: 'spare_part', spare_part_id: spareParts[4].id, item_name: spareParts[4].name, quantity: 1, unit_price: Number(spareParts[4].sell_price) },
    ...
    for (let i = 0; i < txData.length; i++) {
        ...
        try {
            ...
            const existing = await prisma.transactions.findFirst({ where: { invoice_number } });
            if (existing) continue;
